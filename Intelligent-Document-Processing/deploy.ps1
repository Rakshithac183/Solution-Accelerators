...
$credsfilepath = ".\LabValues.txt"

$values = Get-Content $credsfilepath | Out-String | ConvertFrom-StringData
...
$uniqueName= "idp"+$uniqueNumber

$uniqueName = $uniqueName.ToLower();

Write-Host Login to Azure by entering the credentials in the promt... -ForegroundColor Green
Connect-AzAccount
...
CD D:\LabFiles\Intelligent-Document-Processing\deploy

$ScriptRoot = "D:\LabFiles\Intelligent-Document-Processing\deploy"
...
#creating RG Intelligent
New-AzResourceGroup -Name Intelligent -Location $location
...
if ($formsTraining -eq 'true')
{
    ...
    cd D:\LabFiles\Intelligent-Document-Processing\deploy\formstrain
    ...
    cd D:\LabFiles\Intelligent-Document-Processing\deploy\formstrain
		...
		foreach ($subFolder in $subFolders) {
            ...
            $fpath='D:\LabFiles\Intelligent-Document-Processing\deploy\formstrain\' + $folder.Name 
            cd $fpath
...
if ($luisTraining -eq 'true')
{
    ...
    cd D:\LabFiles\Intelligent-Document-Processing\deploy\luistrain 
	foreach ($folder in $folders) {
        cd D:\LabFiles\Intelligent-Document-Processing\deploy\luistrain
...
if ($customVisionTraining -eq 'true')
{
    ...
    cd D:\LabFiles\Intelligent-Document-Processing\deploy\custvisiontrain
	foreach ($folder in $cvFolders) {
		...
		foreach ($subFolder in $cvSubFolders) {
            $fpath='D:\LabFiles\Intelligent-Document-Processing\deploy\custvisiontrain\' + $folder.Name 
            cd $fpath
			$files = Get-ChildItem $subFolder
			foreach($file in $files){
...
cd D:\LabFiles\Intelligent-Document-Processing\deploy\e2etest
